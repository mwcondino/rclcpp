...
void TimersManager::run_timers()
{
  ...
  while (rclcpp::ok(context_) && running_) {
    ...
    // Head timer is reported as cancelled
    if (!time_to_sleep.has_value()) {
      ...
      if (itr == cancelled_timers_.end()) {
        ...
        // Re-heap to move cancelled timer from head
        TimersHeap locked_heap = weak_timers_heap_.validate_and_lock();
        locked_heap.heapify();
        weak_timers_heap_.store(locked_heap);
      }
      // Head timer has already been recongized as cancelled
      // This captures all timers in heap being cancelled
      else {
    ...
    // Head timer is NOT cancelled OR heap is empty
...
void TimersManager::remove_timer(TimerPtr timer)
{
  bool removed = false;
  {
    ...
    // Also remove timer from cancelled_timers_ if present
    auto itr = std::find(cancelled_timers_.begin(), cancelled_timers_.end(), timer);
    if (itr != cancelled_timers_.end()) {
      cancelled_timers_.erase(itr);
    }
